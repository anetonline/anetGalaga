...
if (rawBase.slice(-1) !== "\\" && rawBase.slice(-1) !== "/") rawBase += SEP;
...
/* ------------------------------ Paths & Dirs ------------------------------ */
var DATA_DIR = pathJoin(BASE_DIR, "galaga_data") + SEP;
var PLAYERS_DIR = pathJoin(DATA_DIR, "players") + SEP;
var LEADERBOARDS_DIR = pathJoin(DATA_DIR, "leaderboards") + SEP;
var ART_DIR = pathJoin(BASE_DIR, "art") + SEP;
...
/* ------------------------------ Persistence ------------------------------ */
function playerFile(userNumber){ return pathJoin(PLAYERS_DIR, "player-" + userNumber + ".json"); }
function loadPlayer(userNumber){
    ...
    if(typeof j.totalKills === "undefined") j.totalKills = 0;
    return j;
}
function savePlayer(userNumber,obj){ return writeJsonFile(playerFile(userNumber), obj); }

/* Leaderboard */
function lbFile(){ return pathJoin(LEADERBOARDS_DIR, "galaga-leaderboard.json"); }
...
function playSound(type){
    try{
        switch(type){
            case "shoot": console.beep(1000,30); break;
            case "hit": console.beep(700,40); break;
            case "expl": console.beep(300,100); break;
            case "level": console.beep(1200,120); break;
            case "power": console.beep(1400,60); break;
        }
    }catch(e){}
}
...
function writeScoresAnsi(){
    try{
        ...
        var path = pathJoin(ART_DIR, "galaga-scores.ans");
        var f = new File(path);
...
function writeInstructionAnsi(topic, lines){
    try{
        ...
        var filename = "galaga-instr-" + topic.replace(/\s+/g,"_").toLowerCase() + ".ans";
...
function showHighScoresScroller(){
    ensureDirs();
    writeScoresAnsi();
    var scrollerPath = pathJoin(ART_DIR, "galaga-scores.ans");
...
function startGalaga(userNumber){
    ...
    pdata.totalKills = pdata.totalKills || 0;
    ...
    function collisionsCheck(){
            ...
            for(var j=enemies.length-1;j>=0;j--){
                ...
                if(hitTest(b.x,b.y,e.x,e.y,0.8)){
                    ...
                    if(e.hp <= 0){
                        ...
                        // Increment total kills when enemy is destroyed
                        pdata.totalKills = (pdata.totalKills || 0) + 1;
                        enemies.splice(j,1);
                    }
                    break;
        ...
        for(var m=enemies.length-1;m>=0;m--){
            ...
            if(hitTest(en.x,en.y,player.x,player.y,0.8)){
                if(en.type === "capture"){ player.captured = true; player.captureTimer = 0; addExplosion(en.x,en.y); 
                    // enemy removed on collision: count as a kill
                    pdata.totalKills = (pdata.totalKills || 0) + 1;
                    enemies.splice(m,1); playSound("expl"); 
                }
                else { addExplosion(en.x,en.y); 
                    // enemy removed on collision: count as a kill
                    pdata.totalKills = (pdata.totalKills || 0) + 1;
                    enemies.splice(m,1); pdata.lives--; playSound("expl"); if(pdata.lives <= 0) running = false; }
            }
        }
    }
